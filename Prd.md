...
- 文档页
- 后台资源管理器
- HTML / Markdown 文档上传与静态资源管理
- 标签系统
- 基础搜索
- V1 权限体系：公开、分享可见、登录可见、私密、指定用户、用户组、继承
- 基础无障碍
- 基础安全上线能力
...
- 预览
- 键盘可达的“更多操作”
- 编辑与预览使用弹窗或独立路由，不在资源区右侧常驻堆叠面板
- 资源区根层级不是任一业务文件夹，允许创建顶级文件夹和顶级文档

### 6.7 文档上传
- 支持上传 HTML 或 Markdown 主文件及关联静态资源
- 支持相对路径资源重写
- 支持文件类型白名单
- 支持文件大小限制
- 保留源文档格式时使用隔离渲染，允许文档内交互脚本在 sandbox 中运行，但不得污染站点宿主页面

### 6.8 权限系统
V1 使用统一访问权限，不再把发布状态作为独立前台概念：

- `公开`
- `分享可见`：首页、搜索、栏目列表不可见，但知道分享链接的人可直接访问
- `登录可见`
- `私密`：首页、搜索、栏目列表与分享链接均不可见，仅管理员可管理和预览
- `指定用户`
- `用户组`
...
### 8.3 权限规则
- 文件夹权限默认向下继承。
- `分享可见` 不进入首页、栏目列表和搜索结果，但直接 URL 可访问。
- `私密` 不进入首页、栏目列表和搜索结果，直接 URL 也不可访问。
- 所有前台访问、搜索、直接 URL 命中都必须经过服务端权限校验。
- 无权限访问返回 `404`，而不是 `403`。

### 8.4 发布与预览
- V1 使用权限表达发布可见性：公开/分享可见/登录可见代表可访问内容，私密代表草稿或内部资料。
- 管理员可通过后台打开预览查看私密内容。
- 普通用户永远不能看到私密内容的真实信息。
...
```text
+----------------------------------------------------------------------------------+
| 文览后台         [导入文档] [新建文件夹] [搜索_________] [成员管理] [用户菜单]    |
|----------------------------------------------------------------------------------|
| 左：目录树（含“全部内容”根层级）            | 右：资源区                |
|---------------------------------------------|---------------------------|
| 全部内容                                    | [卡片/列表切换]           |
| ├─ SOP                                      | 标题模版合集  公开   [⋯]  |
| ├─ 教程                                     | 爆款开头库    私密   [⋯]  |
